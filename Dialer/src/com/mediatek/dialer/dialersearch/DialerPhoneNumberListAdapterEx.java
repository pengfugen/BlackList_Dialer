...
public class DialerPhoneNumberListAdapterEx extends PhoneNumberListAdapter {
    ...
    private void bindContactView(View view, Context context, Cursor cursor) {
       ...
       // CharSequence label = CommonDataKinds.Phone.getTypeLabel(context.getResources(), labelType, null);
        ...
        if (photoId > 0) {
            ...
            // pengfugen@wind-mobi.com 2016/03/10 start
			//mContactPhotoManager.loadThumbnail(viewHolder.quickContactBadge, photoId, true, false, request);
            int tempType = -1;
            if (request.subId > 0) {
                if (request.subId == 1) {
                    tempType = ContactsUtils.TYPE_SIM1;
                } else if (request.subId == 2) {
                    tempType = ContactsUtils.TYPE_SIM2;
                }
            } else {
                if (TextUtils.isEmpty(displayName)) {
                    tempType = ContactsUtils.TYPE_NO_NAME;
                } else {
                    tempType = ContactsUtils.TYPE_NORMAL;
                }
            }
            DialerUtils.loadThumbnail(viewHolder.quickContactBadge, photoId, tempType);
            // pengfugen@wind-mobi.com 2016/03/10 end
        }
    ...
    private void bindCallLogView(View view, Context context, Cursor cursor) {
       ...
       // final int simId = cursor.getInt(SIM_ID_INDEX);
        ...
        DialerUtils.loadThumbnail(viewHolder.quickContactBadge, 0, tempType);
        // pengfugen@wind-mobi.com 2016/03/10 end

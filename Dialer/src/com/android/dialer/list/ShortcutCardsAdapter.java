...
public class ShortcutCardsAdapter extends BaseAdapter {
    ...
    public int getCount() {
        /**
         * M: The query limit be changed from 1 to 6 to support conference
         * CallLog. But the shortcut Only need one item. The CallLogAdapter
         * could group its child callLogs into one item. But those callLogs that
         * are not belong to any conference would also be single items. So,
         * check the items count before return to make sure it return at most
         * one.
         * */
		 //M: huangyouzhong@wind-mobi.com 20160310 for Dialer UI feature -s
    	 return 0;
         //return mCallLogAdapter.getCount() > 1 ? 1 : mCallLogAdapter.getCount();
		 //M: huangyouzhong@wind-mobi.com 20160310 for Dialer UI feature -s
    }

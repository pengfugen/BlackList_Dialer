...
import android.os.SystemProperties; // xiaocui@wind-mobi.com add 20150605
...
public class SpecialCharSequenceMgr {
	...
	// xiaocui@wind-mobi.com 20150612 begin
	public static final boolean WIND_DEF_OPTR_E169F_NOS = SystemProperties.get(
			"ro.wind.def.optr.e169f.nos").equals("1");
	// lizusheng@wind-mobi.com 20151217 start
	public static final boolean WIND_DEF_PRO_E200L = SystemProperties.get(
			"ro.wind.def.pro.e200l").equals("1");
	// lizusheng@wind-mobi.com 20151217 end
	// xiaocui@wind-mobi.com 20150612 end
    ...
    static boolean handleSecretCode(Context context, String input) {
        ...
        /// @}
       //add by huangyouzhong@wind-mobi.com 20150828 for bug 81201 start
        if (input.startsWith("*#*#9463*") && input.endsWith("#*#*")) {
            Intent emodeIntent = new Intent("com.wind.intent.action.EMODE");
            emodeIntent.putExtra("com.wind.intent.extra.EMODE_CODE",input);
            context.sendBroadcast(emodeIntent);
            return false;
        }
        //add by huangyouzhong@wind-mobi.com 20150828 for bug 81201 start
        ...
        if(input.startsWith("*") && input.endsWith("#")){
            Log.d(TAG, "---------input.startsWith(*) && input.endsWith(#) =====" + input);

            if ( ( len > 4 )&& !input.startsWith("*#*#")) {
		   	    Intent intent_rec = new Intent("com.wind.intent.action.EMODE");
                        intent_rec.putExtra("com.wind.intent.extra.EMODE_CODE",input);
    ...
    static boolean handleDeviceIdDisplay(Context context, String input) {
        ...
        if (telephonyManager != null && input.equals(MMI_IMEI_DISPLAY)) {
            /*xulinchao@wind-mobi.com 20150805 start*/
            int labelResId;
            if(DialerFeatureOptions.MTK_C2K_SUPPORT){
                labelResId = (telephonyManager.getPhoneType() == TelephonyManager.PHONE_TYPE_GSM) ?
                    R.string.imei : R.string.meid;
            }else{
                labelResId=R.string.imei;
            }
             /*xulinchao@wind-mobi.com 20150805 end*/

            // xiaocui@wind-mobi.com 20150605 begin
            // Modify to add SV Number
            if(WIND_DEF_OPTR_E169F_NOS) {
                labelResId = R.string.imei_sv;
            }
            // xiaocui@wind-mobi.com 20150605 end
            ...
            for (int slot = 0; slot < telephonyManager.getPhoneCount(); slot++) {
                ...
                String imei = null;
                if (WIND_DEF_OPTR_E169F_NOS) {
                    imei = telephonyManager.getDeviceId(slot)+"/"+getSVNumber();
                } else {
			...
			deviceIds = ExtensionManager.getInstance().getDialPadExtension()
					.getSingleIMEI(deviceIds);
			// / @}

			// modified by lizusheng@wind-mobi.com 20151217 start
			if (WIND_DEF_PRO_E200L) {
				deviceIds.clear();
				labelResId = R.string.imei;
				Log.d(TAG,
						"IMEI1:" + SystemProperties.get("ro.wind_imei") + "\n"
								+ "IMEI2:"
								+ SystemProperties.get("ro.wind_imei2") + "\n"
								+ "IMID:"
								+ SystemProperties.get("ro.wind_meid"));
				if (SystemProperties.get("ro.wind_imei") != null) {
					deviceIds.add("SIM1: "
							+ SystemProperties.get("ro.wind_imei"));
				} else {
					deviceIds.add("SIM1: " + imei_invalid);
				}
				if (SystemProperties.get("ro.wind_imei2") != null) {
					deviceIds.add("SIM2: "
							+ SystemProperties.get("ro.wind_imei2"));
				} else {
					deviceIds.add("SIM2: " + imei_invalid);
				}
				if (SystemProperties.get("ro.wind_meid") != null) {
					deviceIds.add("MEID: "
							+ SystemProperties.get("ro.wind_meid"));
				} else {
			...
			AlertDialog alert = new AlertDialog.Builder(context)
					.setTitle(labelResId)
					.setItems(deviceIds.toArray(new String[deviceIds.size()]),
							null).setPositiveButton(android.R.string.ok, null)
					.setCancelable(false).show();
			// modified by lizusheng@wind-mobi.com 20151217 end
			return true;
		}
		return false;
	}

	// xiaocui@wind-mobi.com 20150605 add begin
	// Add SV Number
	private static String getSVNumber() {
		String svNumber = SystemProperties.get("ro.build.sv.number", "01");
		return svNumber;
	}
